library(ggthemes); library(stringr);library(reshape2);library(rstan); library(dplyr); library(ggplot2); library(zoo);
...
# Now run the model
latent_model <- stan(file = "Latent_GDP.stan", 
                  data = list(T = nrow(full_data),
                              Y = full_data_tmp,
                              phi = 0.5), control = list(adapt_delta = 0.99), chains = 4)
...
print(latent_model, pars = c("mu", "rho", "sigma"))
